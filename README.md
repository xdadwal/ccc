...
| `hooks/` | Guardrails: block destructive bash, secret/lockfile edits, secrets in commits, and commits/pushes to `main`/`master`; auto-format on save; inject git status at session start. **Enforced** — they hold even in autonomous runs. |
| `skills/` | `autoloop` (ralph-style autonomy loop with stop conditions), `ship` (plan→verify→commit), `raise-pr` (commit → push → open a focused PR), `update-readme` (keep README true to the code), `sync-config` (pull updates from ccc), `reflect` (learn from interactions → propose config improvements, always surfaced). |
